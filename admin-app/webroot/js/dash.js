...
function getInitialData()
{
	...
	$.ajax({
		url: 'http://localhost/app_data/all.json',
		type: 'GET',
		dataType: 'json',
		data: {}
	})
...
function setupListeners()
{
	...
	$('.addButton').click(function(event) {
		...
		// send
		$.ajax({
			url: 'http://localhost/app_questions/add',
			type: 'POST',
			dataType: 'json',
		...
		.done(function(e) {
			...
			$('.deleteButton').last().click(function(event)
			{
				...
				if(deleteConfirm)
				{
					...
					// send query
					$.ajax({
						url: 'http://localhost/app_questions/delete/'+questionID,
						type: 'DELETE',
						dataType: 'json',
						data: {},
					})
...
function getQuestions()
{
	...
	d3.json("http://localhost/app_questions.json", function(error, json) {
