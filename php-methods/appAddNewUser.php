...
if (isset($_POST['username']) && isset($_POST['school']) && isset($_POST['year'])) {
    $_POST = array_map("strip_tags", $_POST);
    $_POST = array_map("trim", $_POST);
    if ($db->connect_errno > 0) {
        die('Unable to connect to database [' . $db->connect_error . ']');
    } else {
        $sth = $db->prepare("INSERT INTO `app_students`(`name`, `school_name`, `year`) VALUES ('" . $_POST['username'] . "','" . $_POST['school'] . "'," . $_POST['year'].")");
        $OK = $sth->execute();
