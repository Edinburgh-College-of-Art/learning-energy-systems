...
if (isset($_GET['id']) && isset($_GET['date'])) {
    ...
    $query = "select (sum(`light`)+sum(`computer`)+sum(`heater`)+sum(`projector`)) as daySum,date  from `app_data` where `date` >= '" . date('Y-m-d', strtotime("Last Monday")) . "' and `date`<='" . date('Y-m-d', strtotime("This Friday"))  .  "' and `app_students_unique_id`=" . $_GET['id'] . " group by `date` order by `date` ASC";
    $result = mysqli_query($connection, $query);
    while ($r = mysqli_fetch_assoc($result)) {
